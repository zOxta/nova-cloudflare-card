...
<script>
    export default {
        ...
        methods: {
            ...
            onCachePurge() {
                Nova.request().delete('/nova-vendor/nova-cloudflare-card/cache-purge').then(response => {
                    if (!response.data.success) {
                        this.showFailureMessage(response.data.message)
                    } else {
            ...
            showSuccessMessage() {
                this.$toasted.show(
                    this.__('Successfully purged everything cached on CloudFlare!'),
                    {type: 'success'}
                )
            },
            ...
            showFailureMessage(message) {
                this.$toasted.show(
                    !!message ? message : this.__('Could not purge cache on CloudFlare!'),
                    {type: 'error'}
                )
            },
